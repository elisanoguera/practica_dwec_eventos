...
7. Ahora podemos abrir la carpeta del proyecto en `VS Code` y probar su funcionamiento en el navegador (http://localhost:5173/). Deberíamos ver algo así:
...
## Integrar Bootstrap en nuestro proyecto

Para integrar `Bootstrap` un nuestro proyecto `VueJS` seguiremos los siguientes pasos:
...
2. Instalamos `Bootstrap` y sus dependencias usando `npm`:

```sh
npm install bootstrap
```
   
3. Abrimos el archivo `src/main.js` e importamos el archivo `CSS` de `Bootstrap`:
...
Esta práctica tiene como objetivo crear una aplicación `SPA` a través de componentes `VueJS` de un solo archivo. De momento, la aplicación utilizará tres componentes (`Principal (App)`, `MostrarEventos` y `Evento`) que se encargarán de formatear los datos que tendremos almacenados en la aplicación (más adelante utilizaremos datos provenientes de una `API`). Se utilizará Bootstrap para el formato visual. El resultado de esta entrega será parecido a esta imagen:

![Visualización entrega inicial proyecto en el navegador](img/componentes.png)
...
- Plantilla
    - Utiliza el siguiente código `HTML` en la plantilla del componente principal:
     ...
     ```html
       <div>
         ...
         <!-- Insertar aquí el componente MostrarEventos -->

       </div>        
     ```

- Variables de datos
    - `eventos` - será un array de objetos. Crea al menos 2 eventos con todos sus datos. Cada objeto del array `eventos` contendrá la información de un evento determinado. Las propiedades de este objeto serán:
        - `id`: identificador del objeto (*number*)
        - `titulo`: nombre del evento (*string*)
        - `descripcion`: descripción del evento (*string*)
        - `fecha`: fecha del evento (*number*). Almacenará la fecha en formato `timestamp`.
        - `precio`: precio del evento (*number*)
        - `imagen`: imagen del evento (*string*). Almacenará la ruta a la imagen e.g. `src/assets/img1.png` (las imágenes se almacenarán en la carpeta `src/assets`).
    - `mostrar` - Variable auxiliar. En caso de que el array de eventos esté vacío, se mostrará el mensaje `NO HAY EVENTOS DISPONIBLES`.

### Componente `MostrarEventos`

El componente `MostrarEventos` se encargará de mostrar el conjunto de eventos. Crea un nuevo fichero llamado `MostrarEventos.vue` en el directorio `components` que contenga lo siguiente:
     ...
     ```html
        <div class="container">
           <div class="mb-4 mt-4 row row-cols-1 row-cols-md-2 row-cols-xl-3 g-4">
                ...
                <!-- Inserta aquí el listado de eventos -->
...
- Parámetros (props):
    - `eventos` - Parámetro que contiene el array de eventos.

### Componente `Evento`

El componente `Evento` se encargará de mostrar un único evento. Crea un nuevo fichero llamado `Evento.vue` en el directorio `components` que contenga lo siguiente:
...
- Plantilla
    - Utiliza el siguiente código `HTML` en la plantilla:
     ...
     ```html
      <div class="col">
         <div class="card">
            <img class="card-img-top" src="RUTA_IMAGEN">
            <div class="card-body">
            ...
            <div class="card-footer text-muted">
               FECHA (DD/MM/AAAA)
               <span class="float-end">PRECIO €</span>
            </div>
         </div>
      </div>       
     ```
...
- Parámetros (props):
    - `evento` - Parámetro que contiene un objeto con los datos del evento.

- Métodos: 
   - `mostrarFecha` - **Función** que recibirá como parámetro una fecha en formato *timestamp* y devolverá la fecha en *formato localizado* en forma de cadena de texto (función `toLocaleDateString`). Por ejemplo, si `fecha=1775606400000`, la función deberá devolver: `8/4/2026`.
